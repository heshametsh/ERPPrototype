...
| Column Hide/Unhide / visibility | Legacy Tabulator hides visually, keeps definitions/data active, preserves Sort/Filter, records visibility History, saves through explicit Save, and keeps visible Money summaries visibility-aware | RevoGrid Community 4.25.2 source separates full source/proxy data from visible `items`; trim hides items from the rendered collection while keeping them in store, and dimension trim removes hidden column size. Column/filter/sort providers continue to work from authored column identity | Public Pro Column Hide / Column Dialog / Context Menu docs model hidden columns by stable ColumnProp, preserve state across column updates, and keep visibility behind one hide owner | ERP owns year scope, persistence, History/Dirty/Save meaning, last-visible safety, aggregates, and selection reconciliation; Revo adapter owns only visual trim mechanics | Keep full authored columns; store hidden props by year; reapply trim after column rebuild; never implement Hide as Delete or by filtering authored `grid.columns` | REFERENCE PASS COMPLETE - LOCAL-CODE + OFFICIAL-SOURCE + OFFICIAL-DOCS; implementation pending; no licensed PRO-SOURCE |
...
- Current ERP server review found `DepartmentColumnLayout` currently couples Width + IsHidden under one department-scoped RowVersion. The new feature must not make Width year-scoped accidentally.
