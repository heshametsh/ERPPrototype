...
## نقطة الاستقرار الحالية — Phase 8.5

تم اختبار واعتماد منطق تتبع الحقول والتعديل الجماعي ودمج نتيجة الحفظ:

- تعديل عمود لا يشغّل قواعد أعمدة أخرى.
- لصق عمود كامل لا يعيد إطلاق آلاف دورات التعديل المنفصلة.
- حفظ آلاف الصفوف لا يعيد تحديث الخلايا الظاهرة إذا كانت قيم السيرفر هي نفسها المعروضة.
- التنقل بعد اللصق والحفظ يظل مقبولًا بدون تغيير السنة لاستعادة السرعة.
- الأساس يستخدم مفاتيح حقول مستقرة تمهيدًا للأعمدة المخصصة مستقبلًا.

Phase 8.6-R1 تفصل مالك دورة حياة الشيت فقط، ولا تغيّر هذا السلوك.
...
اختبار Phase 8.6-R1 من القسم Q في `Documentation/06_REGRESSION_TEST_CHECKLIST.md`: تغيير السنة عدة مرات، مغادرة الصفحة والرجوع، Resize في صف عميق، والتأكد أن كل أمر يصل إلى شيت واحد فقط. بعد نجاحه نراجع فصل navigation/viewport كخطوة مستقلة، بدون خلطه مع مسار الحفظ أو إضافة ميزة جديدة.
