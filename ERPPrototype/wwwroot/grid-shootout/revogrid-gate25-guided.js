...
const state = {
  candidate: "RevoGrid Community Guided 2.5",
  version: "4.25.2",
  rows: ROWS,
...
async function selectWholeColumn(prop) {
  ...
  if (x < 0) return;
  await grid.setCellsFocus({x,y:0},{x1:x,y1:ROWS-1});
  ...
  return h("span", { class: "erp-header-inner" }, [
    h("span", {
      ...
      onClick: event => {
        ...
        void selectWholeColumn(column.prop);
      }
...
function renderFinal() {
  ...
  if (statuses.every(s => s === "pass")) {
    box.textContent = "النتيجة النهائية: PASS — حل Community المجاني عدى Gate 2.5.";
...
function isWholeBasket(range) {
  const x = visualIndexForProp("basket");
...
function handleFilterTrim(event) {
  ...
  state.notes.push({at:Date.now(),type:"beforefiltertrimmed",hidden,stage:state.stage});
...
function snapshot() {
  ...
  return {
    ...
    gate: "RevoGrid Community Guided Gate 2.5",
