...
$("export").addEventListener("click",()=>{const payload={at:new Date().toISOString(),gate:"RevoGrid Community ERP Gate 4D Manual",version:"4.25.2",rows:data.length,stage,tests,actions,notes,dirty:dirty.size,undo:undoStack.length,redo:redoStack.length,scrollEvents,longTasks:longTasks.length,longTaskMaxMs:longTasks.length?Math.max(...longTasks.map(x=>x.duration)):0,heapMb:performance.memory?performance.memory.usedJSHeapSize/1048576:null,viewport:{width:innerWidth,height:innerHeight,dpr:devicePixelRatio},userAgent:navigator.userAgent};const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`grid-gate4d-manual-${Date.now()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);});
