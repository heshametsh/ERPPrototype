...
async function checkZoom(){
  ...
  if(zoomLeftInitial&&zoomSeen.size>=4&&Math.abs(d-initialDpr)<=.01){setTest("zoom","pass",`Zoom values: ${[...zoomSeen].join(" → ")}. Selection سليم.`);stage="done";instruction("Gate 4B خلص. صدّر JSON وابعتلي الملف.");$("export").disabled=false;}
}
...
function snapshot(){
  ...
  return {at:new Date().toISOString(),gate:"RevoGrid Community ERP Gate 4BB — Undo Fix",candidate:"RevoGrid Community 4.25.2 — Gate 4B",rows:data.length,stage,tests,actions,notes,dirty:dirty.size,undoDepth:undoStack.length,redoDepth:redoStack.length,scrollEvents,longTasks:longTasks.length,longTaskTotalMs:longTasks.reduce((a,x)=>a+x.duration,0),longTaskMaxMs:worstLong,frameGapOver50:frameGaps.filter(x=>x.gap>50).length,maxFrameGapMs:worstFrame,heapMb:performance.memory?performance.memory.usedJSHeapSize/1048576:null,domNodes:document.getElementsByTagName("*").length,viewport:{width:innerWidth,height:innerHeight,dpr:devicePixelRatio},userAgent:navigator.userAgent};
}
