...
async function setGridColumns(cols){
  gridColumns=cloneColumns(cols);
  // Use the documented reactive columns property for structural add/remove.
  grid.columns=cloneColumns(gridColumns);
  await twoFrames();
  return await grid.getColumns();
}
async function applyColumnsOp(op,dir){
  await setGridColumns(dir==="undo"?op.before:op.after);
}
...
async function handleHistory(kind){if(kind==="undo")await doUndo();else await doRedo();
  ...
  else if(stage==="column-undo"&&kind==="undo"){const cols=await grid.getColumns();if(cols.some(c=>String(c.prop)==="erpCustom")){setTest("customColumns","fail","Undo لم يشل العمود.");return;}stage="column-redo";instruction("Undo شال ERP Custom فعليًا ✅. اضغط Ctrl+Y بنفسك.","المفروض العمود يرجع ظاهر قدامك.");}
  else if(stage==="column-redo"&&kind==="redo"){const cols=await grid.getColumns();if(!cols.some(c=>String(c.prop)==="erpCustom")){setTest("customColumns","fail","Redo لم يرجع العمود.");return;}try{await grid.scrollToColumnProp("erpCustom");}catch{} await twoFrames();setTest("customColumns","pass","ERP Custom ظهر + Undo شاله + Redo رجعه فعليًا ✅.");clearHistory();stage="split-select";instruction("دلوقتي اضغط على اسم Basket في الهيدر لتحديد العمود كله.","بعدها هتعمل Split/Full بنفسك.");}
}
...
async function addCustomColumn(){
  ...
  const before=cloneColumns(gridColumns.filter(c=>String(c.prop)!=="erpCustom"));
  const after=[
    ...cloneColumns(before),
    {name:"ERP Custom",prop:"erpCustom",size:170,columnTemplate:headerTemplate}
  ];

  const actual=await setGridColumns(after);
  const exists=actual.some(c=>String(c.prop)==="erpCustom");

  notes.push({
    at:Date.now(),
    type:"custom-column-add-check",
    requested:after.map(c=>String(c.prop)),
    actual:actual.map(c=>String(c.prop)),
    exists
  });

  if(!exists){
    setTest("customColumns","fail","زر الإضافة اتضغط لكن ERP Custom لم يدخل أعمدة RevoGrid.");
    instruction("الإضافة نفسها فشلت قبل Undo/Redo. صدّر JSON وابعتلي الملف.");
    $("export").disabled=false;
    record("manual-add-custom-column",s,false,"column missing immediately after add");
    return;
  }

  pushHistory({kind:"columns",before,after});

  // Show the actual new column to the user before asking for Undo.
  try{ await grid.scrollToColumnProp("erpCustom"); }catch{}
  await twoFrames();

  stage="column-undo";
  setTest("customColumns","running","ERP Custom ظهر فعليًا ✅. اضغط Ctrl+Z.");
  instruction("شايف ERP Custom في الشيت؟ اضغط Ctrl+Z بنفسك.","لا نكمل إلا بعد ما العمود يظهر فعلًا قدامك.");
  record("manual-add-custom-column",s,true,"column verified before history");
}
...
$("export").addEventListener("click",()=>{const payload={at:new Date().toISOString(),gate:"RevoGrid Community ERP Gate 4F Manual — Custom Columns Fix",version:"4.25.2",rows:data.length,stage,tests,actions,notes,dirty:dirty.size,undo:undoStack.length,redo:redoStack.length,scrollEvents,longTasks:longTasks.length,longTaskMaxMs:longTasks.length?Math.max(...longTasks.map(x=>x.duration)):0,heapMb:performance.memory?performance.memory.usedJSHeapSize/1048576:null,viewport:{width:innerWidth,height:innerHeight,dpr:devicePixelRatio},userAgent:navigator.userAgent};const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`grid-gate4f-manual-${Date.now()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);});
