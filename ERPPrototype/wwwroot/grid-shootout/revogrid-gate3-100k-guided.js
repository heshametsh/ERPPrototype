...
function generateRows(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    rows[i] = {
      ...
      basket: baskets[i % 8],
      notes: `Row ${i + 1} — Guided Gate 3 — 100K`,
    };
  }
  return rows;
}
...
async function preparePaste5000() {
  ...
  const targetIds = visible.slice(target, target + 5000).map(row => row.id);
  const values = Array.from({length:5000}, (_,i) => `G3-Paste-${String(i).padStart(5,"0")}`);
  ...
  await grid.setCellsFocus({x:7,y:target},{x:7,y:target});
  grid.focus({preventScroll:true});
  await twoFrames();
  state.pastePrepared = { kind:"paste", target, targetIds, values, startedAt: performance.now() };
  state.stage = "paste-wait";
  $("prepare-paste").hidden = true;
  setTest("paste","running","جاهز على Notes. اضغط Ctrl+V مرة واحدة فقط.");
  setInstruction("اضغط Ctrl+V مرة واحدة فقط. الاختبار هيتحقق من نفس أوامر العمل بالـID حتى لو الشيت مترتب.");
...
async function prepareEndRule() {
  const started = performance.now();
  const visible = await grid.getVisibleSource();
  const target = visible.length - 200;
  const targetIds = visible.slice(target).map(row => row.id);
  const values = Array.from({length:4000}, (_,i) => `G3-End-${String(i).padStart(5,"0")}`);
  const text = values.join("\n");
  try {
    await navigator.clipboard.writeText(text);
  } catch {
  ...
  grid.focus({preventScroll:true});
  await twoFrames();
  state.pastePrepared = { kind:"end", target, targetIds, values, startedAt: performance.now() };
  state.stage = "end-wait";
  $("prepare-end").hidden = true;
  setTest("endRule","running","جاهز على آخر 200 صف ظاهر. اضغط Ctrl+V مرة واحدة.");
  setInstruction("اختبار نهاية الشيت جاهز. اضغط Ctrl+V مرة واحدة؛ المفروض يلصق أول 200 فقط من الـ4000 ولا يضيف صفوف.");
...
async function handlePasteApplied() {
  ...
  if (txn.kind === "paste" && state.stage === "paste-wait") {
    let bad = 0;
    for (let i = 0; i < txn.targetIds.length; i++) {
      const row = byId.get(txn.targetIds[i]);
      if (String(row?.notes ?? "") !== txn.values[i]) bad++;
    }
    const ms = performance.now() - txn.startedAt;
    const ok = txn.targetIds.length === 5000 && bad === 0;
    setTest("paste", ok ? "pass" : "fail", `${ok ? "5000/5000 IDs صحيحة" : `mismatches=${bad}`} | ${ms.toFixed(1)}ms`);
    recordAction("native-paste-5000-by-id", txn.startedAt, ok, `targets=${txn.targetIds.length}, mismatches=${bad}`);
    ...
    const ms = performance.now() - txn.startedAt;
    const ok = source.length === ROWS && txn.targetIds.length === 200 && bad === 0;
    setTest("endRule", ok ? "pass" : "fail", `rows=${source.length.toLocaleString()} | pasted=${txn.targetIds.length - bad}/200 | ${ms.toFixed(1)}ms`);
    recordAction("native-end-rule-4000-to-200-by-id", txn.startedAt, ok, `rows=${source.length}, targets=${txn.targetIds.length}, mismatches=${bad}`);
    state.pastePrepared = null;
    state.stage = "done";
    setInstruction("اختبار الـ100,000 خلص. صدّر JSON وابعتلي الملف.");
    $("export").disabled = false;
    renderFinal();
  }
}
...
function snapshot() {
  ...
  return {
    ...
    gate: "RevoGrid Community Guided Gate 3 — 100K",
