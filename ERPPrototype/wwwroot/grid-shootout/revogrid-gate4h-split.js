...
let cachedSelection=null, pressSelection=null;
...
async function nativeToggle(){
  const s=now();
  const before=pressSelection ? {...pressSelection} : (cachedSelection ? {...cachedSelection} : normalizeRange(await grid.getSelectedRange()));
  pressSelection=null;

  const samples=[{label:"قبل Split",delay:0,range:before,ok:isWholeBasket(before)}];

  $("grid-card").classList.toggle("half");
  samples.push(await sampleSelection("بعد Resize فورًا",0));
...
async function preserveToggle(){
  ...
  if(before){
    ...
    await twoFrames();
    const restoredRange=normalizeRange(await grid.getSelectedRange());
    if(isWholeBasket(restoredRange)) cachedSelection={...restoredRange};
    restored=true;
  }
...
function exportJson(){
  const payload={
    ...
    gate:"RevoGrid Community Gate 4H — Split Focus Fix",
    version:"4.25.2",
    rows:ROWS,
  ...
  a.download=`grid-gate4h-split-${Date.now()}.json`;
  a.click();
  URL.revokeObjectURL(a.href);
}
function captureBeforeButtonFocus(e,kind){
  const current=normalizeRange(grid ? null : null);
  // Use the already-verified logical range cached when Basket was selected.
  // If a current selection is still available, the async pointerdown probe below refreshes it.
  pressSelection=cachedSelection ? {...cachedSelection} : null;
  notes.push({at:Date.now(),type:"split-button-pointerdown",kind,cached:pressSelection});

  // Keep focus in RevoGrid so Phase 1 measures resize only, not button-blur behavior.
  e.preventDefault();

  if(grid){
    grid.getSelectedRange().then(r=>{
      const n=normalizeRange(r);
      if(isWholeBasket(n)){
        pressSelection={...n};
        cachedSelection={...n};
        notes.push({at:Date.now(),type:"pointerdown-range-refresh",kind,range:n});
      }
    }).catch(()=>{});
  }
}

$("nativeToggle").addEventListener("pointerdown",e=>captureBeforeButtonFocus(e,"native"));
$("preserveToggle").addEventListener("pointerdown",e=>captureBeforeButtonFocus(e,"preserve"));
