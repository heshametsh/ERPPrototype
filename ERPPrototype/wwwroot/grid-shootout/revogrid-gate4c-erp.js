...
function renderFinal(){
  ...
  if(s.every(x=>x==="pass")){f.textContent="النتيجة النهائية: PASS — RevoGrid Community عدى ERP Gate 4C.";f.classList.add("pass");$("export").disabled=false;}
...
async function applyCellOp(op,dir){
  ...
  for(const d of op.deltas){
    ...
    if(!row)continue;
    row[d.prop]=useAfter?d.after:d.before;
    refreshDirty(d.id,d.prop);
  }
  // Gate 4B: use the already-proven Gate 2 refresh path after ERP-owned history.
  // A new array identity makes RevoGrid render the restored values reliably.
  grid.source=data.slice();
  await twoFrames();
}
...
function bindGridEvents(){
  ...
  grid.addEventListener("afteredit",e=>{
    ...
    // RevoGrid owns the grid edit; ERP owns its source-of-truth data.
    // Sync the committed value explicitly from the documented afteredit payload.
    const after=d.val!==undefined ? d.val : (d.model?.[p.prop] ?? p.after);
    row[p.prop]=after;

    if(Object.is(p.before,after)){
      setTest("historySave","fail","القيمة لم تتغير فعليًا؛ عدّل Notes لقيمة مختلفة.");
      stage="edit-wait";
      instruction("غيّر Notes لقيمة مختلفة واضغط Enter.");
      return;
    }

    ensureBaseline(p.id,p.prop,p.before);
    refreshDirty(p.id,p.prop);
    pushHistory({kind:"cells",deltas:[{id:p.id,prop:p.prop,before:p.before,after}]});
    notes.push({at:Date.now(),type:"edit-captured",id:p.id,prop:p.prop,before:p.before,after,dirty:dirty.size});
    stage="edit-undo";
    setTest("historySave","running",`التعديل اتسجل وDirty=${dirty.size}. اضغط Ctrl+Z.`);
    instruction("اضغط Ctrl+Z مرة واحدة.","Gate 4C بيتعرف على الزر نفسه حتى لو لغة الكيبورد عربي.");
  });
...
async function checkZoom(){
  ...
  if(zoomLeftInitial&&zoomSeen.size>=4&&Math.abs(d-initialDpr)<=.01){setTest("zoom","pass",`Zoom values: ${[...zoomSeen].join(" → ")}. Selection سليم.`);stage="done";instruction("Gate 4C خلص. صدّر JSON وابعتلي الملف.");$("export").disabled=false;}
}
...
function snapshot(){
  ...
  return {at:new Date().toISOString(),gate:"RevoGrid Community ERP Gate 4C — Keyboard + Store Fix",candidate:"RevoGrid Community 4.25.2 — Gate 4C",rows:data.length,stage,tests,actions,notes,dirty:dirty.size,undoDepth:undoStack.length,redoDepth:redoStack.length,scrollEvents,longTasks:longTasks.length,longTaskTotalMs:longTasks.reduce((a,x)=>a+x.duration,0),longTaskMaxMs:worstLong,frameGapOver50:frameGaps.filter(x=>x.gap>50).length,maxFrameGapMs:worstFrame,heapMb:performance.memory?performance.memory.usedJSHeapSize/1048576:null,domNodes:document.getElementsByTagName("*").length,viewport:{width:innerWidth,height:innerHeight,dpr:devicePixelRatio},userAgent:navigator.userAgent};
}
...
function historyShortcut(e){
  ...
  // `code` is layout-independent: KeyZ/KeyY still work when Windows is on Arabic.
  if(code==="KeyZ" || key==="z" || key==="ئ") return e.shiftKey ? "redo" : "undo";
  if(code==="KeyY" || key==="y" || key==="غ") return "redo";
  return null;
}

window.addEventListener("keydown",e=>{
  if(stage==="readonly-wait"&&e.key==="Enter"){
    setTimeout(()=>verifyReadonly().catch(console.error),0);
    return;
  }

  const kind=historyShortcut(e);
  ...
  // Stop RevoGrid/browser history handling; ERP history owns this shortcut.
  e.preventDefault();
  e.stopPropagation();
  e.stopImmediatePropagation?.();

  handleHistoryKey(kind).catch(err=>{
    console.error(err);
    notes.push({at:Date.now(),type:"history-error",stage,kind,error:String(err?.message||err)});
  });
},true);
